...
// 控制敌人的脚本
public class Bee_Stage2 : MonoBehaviour
{
	// 被击中的动画预制体
    ...
    private void Attack(){
        ...
        float distance1 = Vector3.Distance(Player1.transform.position, transform.position);
        float distance2 = Vector3.Distance(Player2.transform.position, transform.position);
        ...
        Vector3 direction = Position - transform.position + new Vector3(0, m_collider.size.y * 0.5f, 0);
        direction.Normalize();

        GameObject bullet = Instantiate(m_bulletPrefab, transform.position - new Vector3(0, m_collider.size.y * 0.5f, 0), Quaternion.identity);
